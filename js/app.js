...
if (cont === 1) {
  ...
  function isValidCard(num) {
    ...
    for (var contador = 0;contador < array.length;contador++) { // recorre la cadena
      if (contador % 2 === 0) {// ve los indices pares
        array[contador] = array[contador] * 2; // los indices pares lo multiplica por dos
        if (array[contador] >= 10) { // si es mayor o igual a 10
          var digitOne = parseInt(array[contador] / 10); // extrae el primer digito
          var digitTwo = array[contador] % 10;// extrae el segundo digito
          array[contador] = digitOne + digitTwo; //  suma los digitos
        }
      }
      sum += array[contador]; // suma todos los numeros del array
    }
