function isValidCard() {
  ...
  for (var i = input.length - 1; i >= 0; i--) {
    // invertimos los números del input y lo almacenamos en array
    array.push(input[i]);
  }
  for (var j = 0; j < array.length; j++) {
    ...
    if (j % 2 === 1) {
      ...
      if (multipli >= 10) {
        // dividimos entre 10 para conseguir la decenas
        var div = Math.floor(multipli / 10);
        // sacamos el residuo para obtener las unidades
  ...
  for (var index = 0; index < newArr.length; index++) {
    // recorremos el arreglo para obtener la suma de sus elementos
    sum += newArr[index];
  }
  if (sum % 10 === 0) {
    // verificamos si es divisible entre 10 mostrar que la tarjeta es valida
